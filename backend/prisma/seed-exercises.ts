...
function pickTranslation(translations: WgerRaw["translations"], prefer: string[], term?: string): string | null {
...
async function main() {
  console.log(`Seeding up to 100 exercises from wger (${QUERIES.length} queries) — bucket=${BUCKET} supabase=${!!SUPABASE_URL}`);
  let total = 0;
  ...
  for (const q of QUERIES) {
    ...
    for (const raw of raws) {
      ...
      seen.add(raw.uuid);
      const existing = await prisma.exercise.findUnique({ where: { wgerUuid: raw.uuid } });
      if (existing) {
        console.log(`  skip ${raw.uuid} (${existing.name}) already exists`);
        total++;
        continue;
      }
      const name = pickTranslation(raw.translations, ["fr", "en"], q) ?? pickTranslation(raw.translations, ["fr", "en"]) ?? `Exercise ${raw.id}`;
      ...
      total++;
      console.log(`  + ${name} (${raw.uuid}) ${imageUrl ? "[img]" : "[no img]"}`);
      if (total >= 100) break;
    }
    if (total >= 100) break;
