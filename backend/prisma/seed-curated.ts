...
import { CURATED_EXERCISES, FALLBACK_IMAGE_BY_CATEGORY } from "./data/curated-exercises";
...
async function wgerSearch(term: string): Promise<WgerRaw[]> {
  ...
  const list: WgerRaw[] = Array.isArray(data) ? data : (data as { results?: WgerRaw[] }).results ?? [];
  return list;
}

function pickBestImage(images?: WgerRaw["images"]): { url: string | null; thumb: string | null; licenseTitle: string | null; licenseAuthor: string | null } | null {
...
async function main() {
  console.log(`Seed curated — ${CURATED_EXERCISES.length} exercices — supabase=${!!SUPABASE_URL} bucket=${BUCKET}`);
  ...
  for (const curated of CURATED_EXERCISES) {
    ...
    let best: { raw: WgerRaw; img: ReturnType<typeof pickBestImage> } | null = null;
    ...
    for (const term of terms) {
      try {
        ...
        for (const r of raws) {
          const names = (r.translations ?? []).map((t) => t.name.trim().toLowerCase());
    ...
    if (best) {
      ...
      if (usedWgerUuids.has(candidateUuid)) {
        console.log(`  ↳ wgerUuid ${candidateUuid.slice(0, 8)} déjà utilisé pour "${curated.name}" → on garde l'image mais sans wgerUuid`);
        wgerUuid = null;
      } else {
      ...
      console.log(`  ↳ pas d'image wger pour "${curated.name}" → fallback ${curated.category}`);
    }
    ...
    created++;
    console.log(`  + ${curated.name} (${curated.category}) ${imageUrl ? "[img]" : "[no img]"} ${wgerUuid ? wgerUuid.slice(0, 8) : "no-uuid"}`);
  ...
  // Vérif groupement
  const byCat = await prisma.exercise.groupBy({ by: ["category"], _count: true });
  console.log("Par catégorie:", byCat.map((g) => `${g.category}:${g._count}`).join(" "));
}
