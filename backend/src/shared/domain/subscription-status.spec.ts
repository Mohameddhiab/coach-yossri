...
const now = Date.now();
const sub = (
  over: Partial<Parameters<typeof getSubscriptionStatus>[0]> = {},
) => ({
...
describe('getSubscriptionStatus', () => {
  ...
  it('retourne EXPIRE_BIENTOT à 5 jours de la fin', () => {
    expect(
      getSubscriptionStatus(sub({ dateFin: new Date(now + 5 * DAY) })),
    ).toBe('EXPIRE_BIENTOT');
  });

  it('retourne EXPIRE après la fin', () => {
    expect(getSubscriptionStatus(sub({ dateFin: new Date(now - DAY) }))).toBe(
      'EXPIRE',
    );
  });
  ...
  it("retourne EXPIRE si l'abonnement n'a pas encore commencé", () => {
    expect(getSubscriptionStatus(sub({ dateDebut: new Date(now + DAY) }))).toBe(
      'EXPIRE',
    );
  });
});
