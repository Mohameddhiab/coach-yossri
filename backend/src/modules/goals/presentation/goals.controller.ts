...
export class GoalsController {
  ...
  async get(@CurrentUser() auth: AuthUser, @Param("userId") userId: string) {
    const goal = await this.goals.ofMonth(this.resolve(userId, auth), currentMonth());
  ...
  async setWeb(@CurrentUser() auth: AuthUser, @Param("userId") userId: string, @Body() dto: SetGoalDto) {
  ...
  @Roles("COACH")
  @UseGuards(RolesGuard)
  async setMobile(@CurrentUser() auth: AuthUser, @Param("userId") userId: string, @Body() dto: SetGoalDto) {
    return this.set(auth, userId, dto);
  }
  ...
  private async set(auth: AuthUser, userId: string, dto: SetGoalDto) {
    ...
    const goal = await this.goals.replace(resolved, currentMonth(), titre, cible);
    return toGoalApi(goal);
  }

  @Post("users/:userId/goal/checkin")
  async checkin(@CurrentUser() auth: AuthUser, @Param("userId") userId: string) {
    const resolved = this.resolve(userId, auth);
    ...
    const updated = await this.goals.checkin(resolved, currentMonth(), new Date());
